<?php
    ...
    $query = "SELECT * FROM Alertas WHERE idUsuario = ?";
    $stmt = $conn->prepare($query);
    $stmt->bind_param("i", $idUsuario);
    if (!$stmt->execute()) {
        echo ('Error al enviar la solicitud de alertas.');
    }
...
<body>
    <div class="container mb-3">
        ...
        <div>
            <h1 class="text-center mt-3 mb-4">Alertas</h1>
        </div>
        <div class="container mb-3">
                ...
                <table class="table table-bordered">
                    ...
                    <tbody>
                        <?php while ($row = $result->fetch_assoc()): ?>
                            <tr>
                                <td><?php echo htmlspecialchars($row['idAlerta']); ?></td>
                                <td><?php echo htmlspecialchars($row['mensaje']); ?></td>
                                <td><?php echo htmlspecialchars($row['fechaAlerta']); ?></td>
                            </tr>
                        <?php endwhile; ?>
                    </tbody>
                </table>
            <?php else: ?>
                <p>No se encontraron registros de alertas para este usuario.</p>
            <?php endif; ?>
            </div>
        </div>
    </div>
