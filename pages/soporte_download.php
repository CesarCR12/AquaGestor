<?php
    ...
    $query = "SELECT * FROM Soporte WHERE idUsuario = ?";
    $stmt = $conn->prepare($query);
    $stmt->bind_param("i", $idUsuario);
    if (!$stmt->execute()) {
        echo ('Error al enviar la solicitud de soporte.');
    }
...
<body>
    <div class="container mb-3">
        <div>
            <h1 class="text-center mt-3 mb-4">Solicitudes de Soporte</h1>
        </div>
        <div class="container mb-3">
                ...
                <table class="table table-bordered">
                    ...
                    <tbody>
                        <?php while ($row = $result->fetch_assoc()): ?>
                            <tr>
                                <td><?php echo htmlspecialchars($row['idSoporte']); ?></td>
                                <td><?php echo htmlspecialchars($row['mensaje']); ?></td>
                                <td><?php echo htmlspecialchars($row['asunto']); ?></td>
                                <td><?php echo htmlspecialchars($row['fechaTicketSoporte']); ?></td>
            ...
            <?php else: ?>
                <p>No se encontraron registros de soporte para este usuario.</p>
            <?php endif; ?>
            </div>
        </div>
    </div>
